...
def "main hevc" [path: string = ".", --extension(-e): string, --delete(-d), --recursive(-r)] {
    (convert-to
        { |entry, basename|
...
def "main opus" [path: string = ".", --extension(-e): string, --delete(-d), --recursive(-r)] {
    (convert-to
        { |entry, basename|
            let source = $entry.name
...
def "main png" [path: string = ".", --extension(-e): string, --delete(-d), --recursive(-r)] {
    (convert-to
        { |entry, basename|
            let source = $entry.name
...
def "main enum" [path: string = ".", --extension(-e): string, --recursive(-r)] {
    let extension = $extension | str trim --left --char '.'
...
def "main prefix" [path: string = ".", --extension(-e): string, --prefix(-p): string, --recursive(-r)] {
    let extension = $extension | str trim --left --char '.'
